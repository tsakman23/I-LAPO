...
def train_bc(lam: LAOMWithLabels, config: BCConfig):
    ...
    # The actor learns standardized latents; Stage 3 decoder and eval consume the
    # actor's outputs directly, so the pipeline stays consistent with no mean/std
    # bookkeeping downstream. No-op when standardize_targets is False (mean=0, std=1).
